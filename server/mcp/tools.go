...
// torznabDefaultCats mirrors the default of GET /torznab/search.
const torznabDefaultCats = "5000,2000"
...
type serverInfoOut struct {
	...
	JacRedSearch  bool     `json:"jacred_search"`
...
type searchTorrentsIn struct {
	...
	Source string `json:"source,omitempty" jsonschema:"all (default), rutor, torznab, or jacred"`
...
func registerTools(server *mcpsdk.Server) {
	...
	mcpsdk.AddTool(server, &mcpsdk.Tool{
		...
		Description: "Search RuTor, Torznab indexers and/or JacRed (whichever are enabled in settings). Returns magnet links you can pass to add_torrent.",
	}, searchTorrents)
}
...
func getServerInfo(ctx context.Context, req *mcpsdk.CallToolRequest, _ emptyInput) (*mcpsdk.CallToolResult, serverInfoOut, error) {
	_ = ctx
	base := baseURL(req)
	rutorOn, torznabOn, jacredOn, trackTC := false, false, false, false
	if sets := set.BTsets(); sets != nil {
		rutorOn = sets.EnableRutorSearch
		torznabOn = sets.EnableTorznabSearch
		jacredOn = sets.EnableJacRedSearch
		trackTC = sets.TrackTimecode
	}
...
func searchTorrents(ctx context.Context, req *mcpsdk.CallToolRequest, in searchTorrentsIn) (*mcpsdk.CallToolResult, searchTorrentsOut, error) {
	...
	sets := set.BTsets()
	rutorOn := sets != nil && sets.EnableRutorSearch
	torznabOn := sets != nil && sets.EnableTorznabSearch
	jacredOn := sets != nil && sets.EnableJacRedSearch
	// Same default categories as GET /torznab/search: movies + TV.
	torznabSearch := func() []*models.TorrentDetails {
		return torznab.Search(ctx, query, -1, torznabDefaultCats, 0, 0)
	}
	...
	case "jacred":
		if !jacredOn {
			return nil, searchTorrentsOut{Results: []searchHit{}, Message: "JacRed search is disabled"}, nil
		}
		list = jacred.Search(ctx, query)
	case "all":
		if !rutorOn && !torznabOn && !jacredOn {
		...
		if torznabOn {
			list = append(list, torznabSearch()...)
		}
		if jacredOn {
			list = append(list, jacred.Search(ctx, query)...)
		}
	default:
		return nil, searchTorrentsOut{}, fmt.Errorf("source must be all, rutor, torznab, or jacred")
	}
