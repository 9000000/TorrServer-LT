...
type Snapshot struct {
	WhiteIP        Ranger
	BlackIP        Ranger
	Referers       []string // enforced referer hosts: built-in (unless disabled) + user
	...
	IPEnabled      bool
	RefererEnabled bool
	// DefaultReferersEnabled reports whether the built-in referer denylist is
	// enforced.
	DefaultReferersEnabled bool
}
...
type ListsUpdate struct {
	...
	Blacklist string
	Referers  string
	// DefaultReferersEnabled switches the built-in referer denylist; nil
	// keeps the stored choice (clients that only know the three lists).
	DefaultReferersEnabled *bool
}
...
func (s *store) update(u ListsUpdate) (Snapshot, error) {
	...
	disableDefaults := !s.snapshot().DefaultReferersEnabled
	if u.DefaultReferersEnabled != nil {
		disableDefaults = !*u.DefaultReferersEnabled
	}
	if err := settings.SetWAFConfig(settings.WAFConfig{
		...
		DisableDefaultReferers: disableDefaults,
	}); err != nil {
		return Snapshot{}, err
	}
...
func loadLists() Snapshot {
	...
	var defaults []string
	if !cfg.DisableDefaultReferers {
		defaults = defaultBlockedReferers
	}
	referers := mergeReferers(defaults, userReferers)
	...
	return Snapshot{
		...
		DefaultReferersEnabled: !cfg.DisableDefaultReferers,
	}
}
