...
import { Box, Button, CircularProgress, FormControlLabel, FormHelperText, Switch, TextField } from '@material-ui/core'
...
export default function WAFSettings({ onDirtyChange }) {
  ...
  const [defaultReferersEnabled, setDefaultReferersEnabled] = useState(true)
  const [defaultReferers, setDefaultReferers] = useState([])
  ...
  const [initial, setInitial] = useState({ whitelist: '', blacklist: '', referers: '', defaultReferersEnabled: true })
  ...
  const applyResponse = useCallback(data => {
    const next = {
      ...
      // Servers without the switch always enforce the built-in list.
      defaultReferersEnabled: data.default_referers_enabled ?? true,
    }
    setWhitelist(next.whitelist)
    setBlacklist(next.blacklist)
    setReferers(next.referers)
    setDefaultReferersEnabled(next.defaultReferersEnabled)
    setDefaultReferers(data.default_referers || [])
    setInitial(next)
  ...
  const dirty =
    ...
    (whitelist !== initial.whitelist ||
      ...
      referers !== initial.referers ||
      defaultReferersEnabled !== initial.defaultReferersEnabled)
  ...
  const handleSave = async () => {
    ...
    try {
      const { data } = await axios.post(wafHost(), {
        ...
        blacklist,
        referers,
        default_referers_enabled: defaultReferersEnabled,
      })
      applyResponse(data)
  ...
  return (
    <SecondarySettingsContent>
      ...
      <FormControlLabel
        control={
          <Switch
            checked={defaultReferersEnabled}
            onChange={e => setDefaultReferersEnabled(e.target.checked)}
            disabled={readOnly}
            color='secondary'
          />
        }
        label={t('WAF.DefaultReferers')}
        labelPlacement='start'
      />
      <FormHelperText margin='none'>
        {t('WAF.DefaultReferersHint')}
        {defaultReferers.length > 0 && (
          <>
            {' '}
            <span style={{ fontFamily: 'monospace' }}>{defaultReferers.join(', ')}</span>
          </>
        )}
      </FormHelperText>
