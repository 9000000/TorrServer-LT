...
type WAFConfig struct {
	...
	// DisableDefaultReferers turns off the built-in referer denylist. Stored
	// inverted so that a config without the key keeps upstream's behaviour
	// (the list is on).
	DisableDefaultReferers bool `json:"disable_default_referers,omitempty"`
}
...
// readWAFSettings and writeWAFSettings resolve settings.json against the live
// Path rather than the JsonDB singleton's copy of it, so tests (and anything
// else that repoints Path) read the file they expect. The caller holds the
// JsonDB file lock, which is keyed by filename and shared with JsonDB.Set.
...
func writeWAFSettings(db *JsonDB, filename string, root map[string]json.RawMessage) error {
	...
	return os.WriteFile(filepath.Join(Path, filename), data, db.fileMode)
}
