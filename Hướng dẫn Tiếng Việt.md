...
libtorrent được biên dịch với `crypto=openssl` và `webtorrent=on`: mỗi mục tiêu sẽ có một bản OpenSSL tĩnh được build từ mã nguồn (không cần OpenSSL hệ thống) cùng các phụ thuộc WebRTC (libdatachannel/usrsctp/libjuice), cho phép hỗ trợ tracker https/web seeds và WebTorrent (tracker `wss://`, peer trình duyệt). Máy host cần có `cmake` cho các phụ thuộc WebRTC. Tất cả đều được liên kết tĩnh — phụ thuộc động duy nhất trong tệp thực thi cuối cùng là libc/libstdc++/libgcc (trên Windows các thư viện này cũng được liên kết tĩnh). Phiên bản được cố định trong `build/_common.sh` (Boost 1.85.0, libtorrent v2.1.0, OpenSSL 3.5.7) và có thể ghi đè, ví dụ: `LIBTORRENT_TAG=v2.0.13 build/linux-arm64.sh`. Chi tiết đầy đủ và bảng yêu cầu tiên quyết cho từng mục tiêu: [`build/README.md`](build/README.md).
